...
int main() {
  ...
  static_assert(sizeof(char) == 1,
                "size of char MUST be 1"); // @size_of_char_eq_1
  static char buf[64 << 10];
  static_assert(sizeof(buf) > 0,
                "size of buf MUST be positive"); // @sizeof_buf_pos
  static_assert(sizeof(buf) <= SSIZE_MAX,
                "size of buf MUST NOT exceed 0"); // @sizeof_buf_le_SSIZE_MAX
  char *buf_pos = buf;
  // @sizeof_buf_le_SSIZE_MAX => `buf + sizeof(buf)` will not overflow
  for (char *const buf_end = buf + sizeof(buf); buf_pos != buf_end;) {
    // @sizeof_buf_pos => bytes-to-read is not 0
    // @sizeof_buf_le_SSIZE_MAX => bytes-to-read does not exceed SSIZE_MAX
    // @size_of_char_eq_1 => bytes-to-read is not scaled down
    const ssize_t bytes_read = read(fd, buf_pos, buf_end - buf_pos);
    switch (bytes_read) {
    case -1:
      err(1, "error reading file descriptor %d", fd);
    case 0:
      goto eof;
    default:
      buf_pos += bytes_read; // bytes_read is ssize_t => will not overflow
    }
  }
  { // Python for ... else
    char c;
    ...
    case 0:
      break; // goto eof
    default:
      errx(1, "content length exceeds 64 KB"); // FIXME: don't hard code size
    }
  }
eof:
  parse_json(buf, buf_pos);
  // TODO: check parse correctness.
}